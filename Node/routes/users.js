...
router.get('/api/:id',async(req,res)=>{
    try{
        ...
        if(!rows.length){
            return res.status(404).json({success:false, error:'找不到該會員資料'});
        }
        res.json({ success: true, rows });
    }catch(err){
        console.error('取得單一會員資料失敗:', err); // ✅ 印出錯誤訊息
        res.status(500).json({ success: false, error: err.message });
    }
})
...
router.get('/api/order/:id', async (req, res) => {
    try {
      ...
      // 查用戶基本資料
      const [userRows] = await db.query('SELECT user_id, username, full_name FROM users WHERE user_id = ?', [userId]);
      if (!userRows.length) {
